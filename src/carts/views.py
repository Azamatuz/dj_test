...
def cart_update(request):
    ...
    if product_id is not None:
        ...
        cart_obj, new_obj = Cart.objects.new_or_get(request)
        if product_obj in cart_obj.menuitems.all():
            cart_obj.menuitems.remove(product_obj)
            added = False
        else:
            cart_obj.menuitems.add(product_obj) # cart_obj.menuitems.add(product_id)
            added = True
        request.session['cart_items'] = cart_obj.menuitems.count()
